#
# EnsEMBL module for Bio::EnsEMBL::DBSQL::Gene_ObjForeign
#
...
Bio::EnsEMBL::DBSQL::Gene_ObjForeign, specialization of Gene_Obj that
...
Bio::EnsEMBL::DBSQL::Gene_ObjForeign is specialization of Gene_Obj that
allows you to translate table names to other names. The intended use is
for an database that reuses (readonly, not writing!) part of another
database in order to save on speed (no loading of all the assembly related
data) and disk usage.
...
package Bio::EnsEMBL::DBSQL::Gene_ObjForeign;
...
use Bio::EnsEMBL::DBSQL::ObjForeign;

@ISA = qw(Bio::EnsEMBL::DBSQL::ObjForeign 
          Bio::EnsEMBL::DB::Gene_ObjI Bio::Root::RootI
          Bio::EnsEMBL::DBSQL::Gene_Obj);
#
...
#	$f1->seqname($rowhash->{contig});
	...
	if ($anahash{$analysisid}) {
	    ...
	    my $Feature_ObjForeign 
              = Bio::EnsEMBL::DBSQL::Feature_ObjForeign->new(
                -dbobj => $self->_db_obj, 
...
sub write_supporting_evidence {
	...
	my $Feature_ObjForeign = Bio::EnsEMBL::DBSQL::Feature_ObjForeign->new(
                -dbobj => $self->_db_obj, 
                -table_name_translation => $self->_table_name_translations);
