...
package Bio::EnsEMBL::DBSQL::Protein_Feature_Adaptor2;
...
sub fetch_by_feature_and_dbID{
    ...
    $sth = $self->prepare ("select p.seq_start,p.seq_end,p.analysis,p.score,p.perc_id,p.evalue,p.hstart,p.hend,p.hid from protein_feature p,analysis a where a.id = p.analysis and p.translation = '$transl' and p.analysis = a.id and a.gff_feature != 'domain' and a.db = '$feature'");
