    ...
    if ( defined($arr) ) {
      ...
      unless ( @row ) {
        ...
        if (defined $coord_system_name && defined $cs) {
          # if $coord_system_name eq "chromosome" it may be aliased to some
          # other coord system that happens to have karyotypes.
          # check to see if this is the case, and substitute in the
          # "real" coord_system name in place of "chromosome" if needed

          if (($cs->alias_to eq "chromosome") &&
              ($coord_system_name eq "chromosome")) {
            $syn_sql .= "AND cs.name = '" . $cs->name() . "' ";
          } else {
        ...
        # Case 1 - A synonym exactly matches the query, and either:
        #     a - the coord_system matches 
        #               or
        #     b - the user is looking for toplevel
        #   In this case, we now have the real seq_region name, so we can re-run
        #   fetch_by_region using that name, coord_system, and version
        # Case 2 - A synonym exactly matches, but none of the coord_systems match
        #   and the user is not looking for toplevel.
        #   In this case, we return nothing, and move on to try fuzzy-matching
        #   if requested.
        # Case 3 - No synonyms match, so repeat the query using wildcards
        ...
        if ($matched_synonym == 0) {
          ...
          if (scalar(@matched_syn_wrong_cs) > 0) {
            my $cs_warning_text = "Searched for a known feature on coordniate system: " .
                                  $cs->dbID .
                                  " but found it on " .
                                  join(", ", map {if ($_->[1]) {
                                                     $_->[0] . ":" . $_->[1]
...
sub _create_chromosome_alias {
  ...
  if ( $self->{'karyotype_cache'} ) {
    ...
    # get candidate coordSystem(s) to alias
    foreach my $seq_region_name ( keys %{$karyotype_seq_regions} ) {
      foreach my $coord_system_id ( keys %{ $karyotype_seq_regions->{ $seq_region_name } } ) {
        my $rank = $karyotype_seq_regions->{ $seq_region_name }->{ $coord_system_id }->{ rank };
  ...
  if ( !$cs_id ) {
    ...
    my $cs = $csa->fetch_by_dbID( $cs_id );
    if ( !$cs ) {
      throw("Unable to retrieve CoordSystem object using dbID: $cs_id");
    }
