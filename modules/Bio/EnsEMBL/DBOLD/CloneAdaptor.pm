...
Bio::EnsEMBL::DBOLD::CloneAdapor

=head1 SYNOPSIS

    # $db is Bio::EnsEMBL::DB::Obj 

    my $da= Bio::EnsEMBL::DBOLD::CloneAdaptor->new($obj);
...
package Bio::EnsEMBL::DBOLD::CloneAdaptor;
use vars qw(@ISA);
use strict;
   ...
   my $sth = $self->prepare("
    ...
    while (my $rowhash = $sth->fetchrow_hashref) { 
        ...
        if( ! exists $got{$rowhash->{'gene'}}) {  
           ...
           my $gene_obj = Bio::EnsEMBL::DBOLD::Gene_Obj->new($self->db);             
