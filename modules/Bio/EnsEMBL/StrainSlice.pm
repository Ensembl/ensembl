...
=head1 SYNOPSIS
   ...
   foreach my $vf (@{$differences}){
      print "There is a difference between $strain_name and $strain_name2 at ", $vf->start,"-",$vf->end,
            " with allele ", $vf->allele_string(),"\n";
   }
...
sub new{
    ...
    #get the VariationFeatures in the Strain
...
sub get_all_differences_StrainSlice{
	...
	#need to create a copy of VariationFeature for the first Strain
	...
	#need to create a copy of VariationFeature, but changing the allele by the allele in the reference sequence
...
#for a given VariationFeature, converts the allele into the reference allele and returns
#the converted VariationFeature
...
#  $new_strand = $results_ordered[-1]->strand() if (ref($results_ordered[-1]) eq 'Bio::EnsEMBL::Mapper::Coordinate');
  ...
  #update the VariationFeatures in the sub_Slice of the Strain
   my @results;
  my %vf;
  my $new_variation_feature;
  foreach my $variationFeature (@{$self->{'variationFeatures'}}){      
      $new_variation_feature = $variationFeature->transfer($subSlice);
      #only transfer the coordinates to the SubSlice that are within the boundaries
      if ($new_variation_feature->start >= 1 && $new_variation_feature->end <= $subSlice->length){
	  push @{$new_variations}, $new_variation_feature;
      }
  }
...
sub mapper{
	...
	#there is a new variationFeature in the Strain
	foreach my $variation_feature (@variation_features_ordered){
		...
		$length_allele = $variation_feature->length + $variation_feature->length_diff(); #length of the allele in the Strain		    
		$end_slice = $variation_feature->start() - 1; #set the end of the slice before the variationFeature
		...
		$start_slice = $end_slice + $variation_feature->length+ 1; #set the beginning of the slice after the variation feature
	    }
	}
