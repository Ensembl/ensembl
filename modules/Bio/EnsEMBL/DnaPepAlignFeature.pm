...
= head1 SYNOPSIS
  ...
  my $feat = new Bio::EnsEMBL::DnapepeAlignFeature(-seqname => 'myseq',
						  -start   => 100,
						  -end     => 120,
						  -strand  => 1,
						  -hstart  => 200,
  ...
  #
  # Note that the hit  lengths are a factor of 3 less than the query
  # as we have a dna-pep alignment
...
=head2 cigar_string
    ...
    Arg      : Nothing (for just returning an existing cigar_string) or
               ...
               Its format is qstart,qend,querylength*strand.
    ...
    Usage    : $dna_feat->cigar_string('100,200,3:109,203,12');
    ...
    Function : This module is for storing gapped dna-protein alignments.  To save 
               ...
               are the minimum and maximum query coords of all the gapped pieces.
...
sub _parse_cigar {
  ...
  foreach my $piece (@pieces) {
    ...
    if ($length%3 != 0) {
      $self->throw("Can't have an ungapped dna-pep alignment that has fractional residues - length is [$length]");
    }
