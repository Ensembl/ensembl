...
# Reverting mocked species_id to its original value. Now the same functions within this block should work again.
  $asia->is_multispecies(0);
  $asia->species_id(1);

  $asi= $asia->fetch_by_stable_id("T1");
  is( $asi->release, 2, "T1 is from release 2");

  @assoc = @{ $asia->fetch_associated_archived($asia->fetch_by_stable_id_version( "G2", "2" )) };
  ...
  $aRef = $asia->fetch_successors_by_archive_id($asia->fetch_by_stable_id_dbname( "G4", "release_1" ));
  is( scalar( @$aRef ), 1, "G4 has 1 successors" );
  
  $aRef = $asia->fetch_history_tree_by_stable_id("G1");
