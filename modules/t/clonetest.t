...
#Check if its get an existing clone (latest embl_version)
eval {
    $clone=$db->get_Clone("manu-test");
};
if ($@) {
    print "not ok 4\n";
    print STDERR "Could not get an existing (latest) clone using get_clone\n";
}
else {
    print "ok 4\n";
}

#Check its got the latest (highest embl_version) clone
if ($clone->embl_version == 2 ) {
    print "ok 5\n";
}
else {
    print "not ok 5\n";
    print STDERR "clone->embl_version does not give the expected value!\n";
}

#Check if its get an existing clone with embl_version
eval {
    $clone=$db->get_Clone_by_version("test3", 1);
};
if ($@) {
    print "not ok 6\n";
    print STDERR "Could not get an existing clone/version using get_clone\n";
}
else {
    print "ok 6\n";
}
