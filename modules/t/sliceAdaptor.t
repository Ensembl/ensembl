...
### test chromosome alias is not created for a requested human chromosome slice
...
### test chromosome alias is created ok for a requested parus_major chromosome slice using a synonym
my $name_synonym = "CM003709.1";
my $name_wildcard = "CM003709";
my $p_major_synonym_slice = $parus_major_sa->fetch_by_region($alias, $name_synonym);
is($p_major_slice->coord_system()->alias_to(), $alias, "Correctly retrieved alias '$alias' using synonym '$name_synonym'");

my $p_major_wildcard_slice = $parus_major_sa->fetch_by_region($alias, $name_wildcard);
is($p_major_wildcard_slice->coord_system()->alias_to(), $alias, "Correctly retrieved alias '$alias' using synonym wildcard '$name_wildcard'");
