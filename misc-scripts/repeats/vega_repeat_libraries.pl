...
foreach my $db (@dbs) {
  ...
  $dbh->do("use $db");
  if ($db =~ /vega_mus_musculus/) {
      print STDERR "Making vega mouse specific changes\n\tcreating backup table repeat_consensus_script_backup, just in case!\n";
      $dbh->do("drop table repeat_consensus_script_backup");
      $dbh->do("create table repeat_consensus_script_backup select * from repeat_consensus");
      print STDERR "\tcopying repeat_name to repeat_consensus\n";
      $dbh->do("update repeat_consensus set repeat_consensus = repeat_name where repeat_class = 'Tandem_repeat'");
      print STDERR "\tsetting repeat_name to \'trf\' where appropriate\n";
      $dbh->do("update repeat_consensus set repeat_name = 'trf' where repeat_class = 'Tandem_repeat'");
  }
