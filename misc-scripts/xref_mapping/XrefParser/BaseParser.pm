...
#Some data_uris look like:
# 
# script:wget=>http://www.genenames.org/cgi-bin/hgnc_downloads.cgi?title=HGNC+output+data&hgnc_dbtag=on&col=gd_hgnc_id&col=gd_status&col=gd_ccds_ids&status=Approved&status_opt=2&level=pri&=on&where=&order_by=gd_app_sym_sort&limit=&format=text&submit=submit&.cgifields=&.cgifields=level&.cgifields=chr&.cgifields=status&.cgifields=hgnc_dbtag,host=>ens-livemirror,dbname=>ccds_human_68,
# script:wget=>http://www.ebi.ac.uk/gxa/dataExport/organisms,project=>ensembl,
#
# If your URI starts with script: this will return all key/value pairs in a hashref

sub parse_opts_from_file {
  my ($self, $data_uri) = @_;
  my $opts = {};
  if($data_uri !~ /^script:/) {
    return $opts;
