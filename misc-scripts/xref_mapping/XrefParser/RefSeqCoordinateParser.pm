...
XrefParser::RefSeqCoordinateParser
...
=head1 SYNOPSIS
  ...
  my $parser = XrefParser::RefSeqCoordinateParser->new($db->dbh);
...
sub run_script {
  ...
  if ( (!defined $source_id) || (!defined $species_id) || (!defined $file) ){
    confess "Need to pass source_id, species_id and file as pairs";
  }
  ...
  unless ( defined $db || ( ($file_params->{project} eq 'ensembl') || ($file_params->{project} eq 'ensemblgenomes') ) ) {
    confess "Missing or unsupported project value (supported values: ensembl, ensemblgenomes), or missing db value.";
  }
    ...
    foreach my $gene_of (@{$genes_of}) {
      ...
      foreach my $transcript_of (sort { $a->start <=> $b->start } @{$transcripts_of}) {
        ...
        if ($best_id) {
          ...
          $self->add_direct_xref($xref_id, $best_id, 'Transcript', undef);
          ...
          if (defined $tl && defined $tl_of) {
            if ($tl_of->seq eq $tl->seq) {
              ...
              $self->add_direct_xref($tl_xref_id, $tl->stable_id(), 'Translation', undef);
            }
          }
        }
      }
...
=head2 parse_file_string
  ...
  Description: parses the input string $file into an hash.
               ...
               Returns a hash built with keys=>values provided.
  Return type: Hash
  Caller     : internal
=cut
