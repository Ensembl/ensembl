...
AND a.logic_name='percentgc'), -PARAMS=>[$db->species_id()] );
...
-SQL=>"select distinct display_label, description from analysis_description where is_current = 1 and logic_name = 'percentgc'"
			) };
