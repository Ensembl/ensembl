...
if ($db =~ /([\w\_]+)_(core|otherfeatures)_([\d\_\w]+)/) { ####disable otherfeatures for now as in plants core and otherfeatures might share the ids .. 
	#if ($db =~ /([\w\_]+)_(core)_([\d\_\w]+)/) { ### CHANGED
...
# 1 comes from species_id = 1 in meta table
	...
	my $t1 = time;
	#my $dba = db_connect($dbname, $readDB) ;
...
sub load_ids{
    ...
    foreach my $object_name (@stable_id_objects) {
    	...
    	if ($object_name =~ /([A-Za-z]+)Archive/) {
    		...
    		print "Number of rows in $object => $count\n";
...
sub build_insert_sql{
	...
	#print "====================Number of rows to be inserted $rows========\n";
	...
	while(my $ref = $import_sth->fetchrow_arrayref) {
		...
		if ($rows_in_insert < $max_rows_in_insert){
			#print "Reached if  $rows_in_insert <=> $max_rows_in_insert \n";
			$insert_values .= ',' if $start++;
			$insert_values .= '('
			. (join (",", map { $dbh_read->quote($_)} @{$ref}))
			.')';
			$rows_in_insert++;
		}else{
			#print "Reached else  $rows_in_insert <=> $max_rows_in_insert \n";
			$insert_values .= ',' if $start++;
			$insert_values .= '('
			. (join (",", map { $dbh_read->quote($_)} @{$ref}))
			.')';
			...
			#print "Number of elements in array : ", scalar(@insert_container), "\n";
			$insert_values = undef;
			$rows_in_insert = 0;
			$start=0;
		}
	...
	if ($insert_values){
		#print "Reached OUTER if  $rows_in_insert <=> $max_rows_in_insert \n";
		my $insert_sql_with_values = $insert_sql . $insert_values;
		push(@insert_container ,$insert_sql_with_values);
	}
...
#    my $t = time;
	...
	if ($object_name =~ /([A-Za-z]+)Archive/) {
            ...
            my $sql = qq(INSERT INTO $writeDB->{'dbname'}.archive_id_lookup SELECT DISTINCT old_stable_id, species_id + $offset, '$dbtype', '$object' FROM stable_id_event
                                        WHERE old_stable_id IS NOT NULL
                                          AND type = '$lc_object'
...
# Archive IDs will not work as we dont have species_id column populated
	    ...
	    $sql = qq{INSERT INTO $writeDB->{'dbname'}.stable_id_lookup SELECT DISTINCT o.stable_id, cs.species_id + $offset, '$dbtype', '$object_name' FROM $object o LEFT JOIN transcript t USING (transcript_id) LEFT JOIN seq_region sr USING(seq_region_id) LEFT JOIN coord_system cs USING(coord_system_id)};

	} else {
	    $sql = qq{INSERT INTO $writeDB->{'dbname'}.stable_id_lookup SELECT DISTINCT o.stable_id, cs.species_id + $offset, '$dbtype', '$object_name' FROM $object o LEFT JOIN seq_region sr USING(seq_region_id) LEFT JOIN coord_system cs USING(coord_system_id)};
	}
#	warn "\t SQL: $sql\n";
	eval {
	    #$dbh->do($sql);####CHANGED
