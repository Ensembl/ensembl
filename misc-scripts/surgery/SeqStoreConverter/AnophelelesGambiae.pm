...
sub create_coord_systems {
  ...
  $self->debug("AnophelesGambiae Specific: creating scaffold and " .
  ...
  my @coords =
    (['chromosome', $ass_def, 'top_level,default_version'],
     ["scaffold" , $ass_def, "default_version,sequence_level"]);
...
#
# getter/setter needed by anopheles to perform coord transforms
...
sub create_seq_regions {
  ...
  $self->debug("AnophelesGambiae Specific: creating scaffold seq_regions");
  ...
  my $sth = $dbh->prepare
    ("SELECT c.contig_id, c.name, " .
     "c.embl_offset, c.dna_id "  .
     "FROM $source.contig c ");
  $sth->execute();

  my %contig_lists;

  my($contig_id, $name, $length, $dna_id);
  $sth->bind_columns(\$contig_id, \$name, \$offset, \$length, \$dna_id);

  while($sth->fetch()) {
    my $position;
    ($name, $position) = split(/\_/, $name);
    $contig_lists{$name} ||= [];
    $contig_lists{$name}->[$position-1] = [$contig_id,$offset,$length
					   $dna_id];
  }
  ...
  foreach my $ctgname (keys %contig_lists) {
    ...
    #length of scaffold is length of last contig plus its offset
    my $last_contig = $ctg_list->[-1];
    my $length = $last_contig->[1] + $last_contig->[2] - 1;

    foreach my $contig (@$ctg_list) {
      my $old_id = $contig->[0];
      $contig_mappings{$old_id} = [$new_id,$length];
      $dna_mappings{$contig->[3]} = $new_id;
    }
  ...
  #
  $self->debug("AnophelesGambiae Specific: loading scaffold seq regions");
  ...
  ###
  ### TBD have to make new assembly table from scaffolds to chromosomes
  ### and probably chunks to scaffolds as well
  ###
  ###
