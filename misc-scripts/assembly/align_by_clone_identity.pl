...
my $fmt6 = "%-10s%-12s%-10s%-12s\n";
  ...
  foreach my $R_chr ($support->sort_chromosomes($R_chrlength)) {
    ...
    $support->log_stamped("Chromosome $R_chr...\n", 1);

    # skip chromosomes not in both sets
    my $A_chr = $ref_chr_map->{$R_chr};
    unless ($A_chrlength->{$A_chr}) {
      $support->log("No equivalent chromosome in alternative assembly. Skipping.\n", 1);
      next;
    }

    # fetch chromosome slices
    my $R_slice = $R_sa->fetch_by_region('chromosome', $R_chr, undef, undef, undef, $support->param('assembly'));
    ...
    $support->log(sprintf($fmt5, "Alternative chromosome name:", $A_chr), 2);
