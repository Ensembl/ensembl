...
my $length = $clone->seq->seq_len;
print "   <seq length=\"$length\" type=\"DNA\" id=\"".$clone->id."\">\n";
print "      <description>\n";
print "         Sequence entry: ".$clone->id."\n";
print "      </description>\n";
print "      <name>".$clone->id."</name>\n";
print "      <version>".$clone->version."</version>\n";
print "      <creation_date>".localtime($clone->created)."</creation_date>\n";
print "      <modification_date>".localtime($clone->modified)."</modification_date>\n";
print "   </seq>\n";
    
foreach my $contig ($clone->get_all_Contigs()) {
print STDERR "Dumping contig ".$contig->id."\n";
#Get supporting features
my @features;
push(@features,$contig->get_all_SimilarityFeatures);

#Print contig header
my $length= $contig->seq->seq_len;
...
my $length=$trans->translate->seq_len;
my $nuc = 60;
my $whole = int($length / $nuc) * $nuc;
my( $i );
for ($i = 0; $i <= $whole; $i += $nuc) {
