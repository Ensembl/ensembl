...
sub map_coords {
    ...
    foreach my $curr ( @{$contigs} ) {
        if (defined $prev 
            ...
            if ( $fpcctg_start == 1 )  {
                ...
                return ('missing', -999, -999, 'missing');
            }
            #else:
    ...
    return ('bug2', -999, -999, 'bug?');
}


while(<>)  {

    my ($chr, $source, $tag, $start, $end, $score, $strand, $frame, @rest)  = split("\t");
...
###    ($chr, $start, $end) = $stadaptor->convert_chromosome_to_fpc($chr,$start, $end);
    ...
    if ($status ne 'missing') { 
        print join("\t", ($contig, $source, $tag, $ctg_start, $ctg_end, $score, $strand, $frame, $rest)), "\n";;

        if ($status eq 'init' ) {
            warn "$chr: assumed that $rest lies on missing initial part of $contig";
        }
    } else  {# ($status eq 'missing')
        warn "$chr: cannot find contig for $rest; ignoring it";
    }
}
